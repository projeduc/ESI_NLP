...
\begin{itemize}
	...
	\item \optword{dorsal}: Posterior part of the tongue. 
	
	E.g., \expword{\textipa{[c], [k], [g], [q],} \RL{.g} \textipa{[G]}}.
...
\begin{itemize}
	...
	\item Writing systems (according to their \keyword{graphemes}): 
	\begin{itemize}
...
\begin{itemize}
	...
	\item \keyword{Morpheme}: the smallest unit of language which (\textcolor{yellow}{not witch}) has its own meaning. 
...
\begin{itemize}
	\item A \keyword{sentence} is composed of several \keyword{phrases}; each includes words and other phrases.
	...
	\item Eg. \expword{The students understood the course very well}
	\begin{itemize}
		\item \expword{The students}: A nominal phrase.
		\item \expword{understood the course very well}: A verbal phrase.
		\item \expword{the course}: A nominal phrase.
		\item \expword{very well}: A adjectival phrase.
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \optword{Dependency grammar}: a set of binary relations between a sentence's words.
...
\begin{itemize}
	...
	\begin{CJK}{UTF8}{min}
		\begin{itemize}
		\item \expword{川} (river), \expword{山} (mountain), 
		
		\item \expword{音} (sound, noise) + \expword{楽} (music, comfort, ease) = \expword{音楽} (music)
		\end{itemize}
	\end{CJK}
...
\begin{itemize}
	...
	E.g. \expword{Karim, who is lazy, has stopped writing. \textrightarrow\ 
		Karim was writing. Karim is lazy. Karim no longer writes. There is an entity called Karim.}
...
\begin{itemize}
	\item \optword{Word-sense disambiguation}: identifying a word's meaning.
...
\begin{itemize}
	...
	\item \optword{Coherence}: looking for relations between sentences.
	\item \optword{Ellipsis resolution}: finding omitted elements of a text.
	Example of an ellipsis: \expword{``Peter eats cherries, Paul strawberries"}.
\end{itemize}
