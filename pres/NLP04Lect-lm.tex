...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item $P(\text{\textit{he wants to swim}}) = 
			\underbrace{P(he|\text{\textit{\textless s\textgreater}})}_{\frac{3}{4}}\ 
			\underbrace{P(wants|he)}_{\frac{2}{3}}\ 
			\underbrace{P(to|wants)}_{\frac{1}{2}}\ 
			\underbrace{P(swim|to)}_{\frac{0}{1}}\ 
			\underbrace{P(\text{\textit{\textless/s\textgreater}}|swim)}_{\frac{1}{1}} = 0$
		\end{itemize}
...
\begin{frame}
	...
	\begin{exampleblock}{Example: Laplace smoothing}
		\begin{itemize}
			...
			\item If you add new tokens, you have to count them into the vocabulary
			\item $|V| = 9 + 2 = 11$ (Counting start and end markers)
			\item $P(\text{\textit{he wants to swim}}) = 
			\underbrace{P(he|\text{\textit{\textless s\textgreater}})}_{\frac{3 + 1}{4 + 11} = \frac{4}{15}}\ 
			\underbrace{P(wants|he)}_{\frac{2 + 1}{3 + 11} = \frac{3}{14}}\ 
			\underbrace{P(to|wants)}_{\frac{1 + 1}{2 + 11} = \frac{2}{13}}\ 
			\underbrace{P(swim|to)}_{\frac{0 + 1}{1 + 11} = \frac{1}{12}}\ 
			\underbrace{P(\text{\textit{\textless/s\textgreater}}|swim)}_{\frac{1 + 1}{1 + 11} = \frac{2}{12}} 
			= \frac{1}{2 * 3^2 * 5 * 7 * 13} $
		\end{itemize}
	\end{exampleblock}
...
\begin{frame}
	...
	\begin{block}{Interpolation (Tri-grams as example)}
		...
		\begin{itemize}
			\item P(\text{\textit{\textless/s\textgreater}}) will be calculated; so "\textless/s\textgreater" must be considered in uni-grams
		\end{itemize}
	\end{block}
	...
	\begin{exampleblock}{Example: Bi-gram interpolation}
		\begin{itemize}
			...
			\underbrace{P(he|\text{\textit{\textless s\textgreater}})}_{(0.8\frac{3}{4} + 0.2\frac{3}{21})}\ 
			\underbrace{P(wants|he)}_{(0.8\frac{2}{3} + 0.2\frac{2}{21})}\ 
			\underbrace{P(to|wants)}_{(0.8\frac{1}{2} + 0.2\frac{1}{21})}\ 
			\underbrace{P(swim|to)}_{(0.8\frac{0}{1} + 0.2\frac{1}{21})}\  
			\underbrace{P(\text{\textit{\textless/s\textgreater}}|swim)}_{(0.8\frac{1}{1} + 0.2\frac{4}{21})} = $\\
...
\begin{itemize}
	\item Language models are probabilistic models over a unit (word, character, etc.) given some context (some words or characters in its proximity).
	...
	\begin{itemize}
		\item Simple MLPs
		\item RNNs
		\item Transformers (\keyword{Already seen in chapter 02})
	\end{itemize}
\end{itemize}
