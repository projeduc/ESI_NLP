...
\begin{frame}
	...
	\begin{minipage}{.56\textwidth}
		...
		\begin{algorithm}[H]
			...
			Add the rule $v \rightarrow\ \sigma$ to $P'$\;
			...
			\For(\tcp*[f]{Browse rows from bottom to top (no diagonal cells)}){$ i = (n-1) \ldots 2$}{
				\For(\tcp*[f]{Browse columns from diagonal+1 to end}){$ j = (i+1) \ldots n $}{
					\For(\tcp*[f]{Combine left and right spans via rules A → B C}){$ k = i \ldots (j-1) $}{
						\ForEach{$A$ such as $(A \rightarrow B C) \in P $, $B \in T[i, k]$ and $C \in T[k+1, j]$}{
...
\begin{frame}
	...
	\begin{itemize}
		\item Given a probabilistic context-free grammar (PCFG) $G = (\Sigma, N, P, S)$,
	...
	\begin{block}{Probabilistic CKY: Sentence Recognition}
		...
		\begin{algorithm}[H]
			...
			\For{$ i = (n-1) \ldots 1$ }{
				\For{$ j = (i+1) \ldots n $}{
					\For{$ k = i \ldots (j-1) $}{
						\ForEach{$A$ such as $(A \rightarrow B C) \in P $, $T[i, k, B] > 0$ and $T[k+1, j, C] > 0$}{
