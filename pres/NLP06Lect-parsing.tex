...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item E.g. \expword{The/\textsubscript{DT} course/\textsubscript{NN} is/\textsubscript{VB} interesting/\textsubscript{AJ}}.
		\end{itemize}
		\item One or many words having certain POS form a \keyword{phrase}.
		\begin{itemize}
			\item E.g. \expword{[ The/\textsubscript{DT} course/\textsubscript{NN} ]\textsubscript{NP} [ is/\textsubscript{VB} interesting/\textsubscript{AJ} ]\textsubscript{VP}}.
		\end{itemize}
	\end{itemize}
	
	\begin{center}
%		\includegraphics[width=0.3\textwidth]{../img/intro/const_gram.pdf}
		\hgraphpage[0.3\textwidth]{const_gram.pdf}
	\end{center}
...
\begin{frame}
	...
	\begin{itemize}
		\item $G = (\Sigma, N, \mathcal{P}, S)$ is a grammar.
		\item $\Sigma$ is the set of terminal symbols (vocabulary).
		\begin{itemize}
			\item E.g. \expword{$\Sigma$ = \{the, little, cat, eats, a, fish, ...\}}.
		\end{itemize}
		\item $N$ is the set of non-terminal symbols (variables).
		\begin{itemize}
			\item E.g. \expword{$N$ = \{S, NP, VP, DT, N, AJ, ...\}}.
		\end{itemize}
	\end{itemize}
	\end{minipage}
	...
	\begin{itemize}
		...
		\item $\mathcal{P}$ is the set of production rules.
		\item Production rules have the form $A \rightarrow \beta$, where $A \in N$ and $\beta \in (\Sigma \cup N)^*$.
		\begin{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		\item \textbf{Problem}: CFGs may generate multiple parse trees for the same sentence.
		\item \textbf{Solution}: Assign a probability to each production rule.
		\item Let $G = (\Sigma, N, \mathcal{P}, S)$ be a grammar.
		\item Rules have the form $A \rightarrow \beta \; [p]$, where $A \in N$ and $\beta \in (\Sigma \cup N)^*$.
		\item $p$ is the probability of the rule.
		\item This probability is estimated using an annotated corpus (\keyword{Treebank}).
	\end{itemize}
	
	\begin{block}{Rules probability estimation}
		\[
		P(A \rightarrow \beta ) = P(\beta \mid A ) = \frac{C(A \rightarrow \beta)}{C(A)}
		\]
	\end{block}
...
\begin{frame}
	...
	\begin{itemize}
		\item \keyword{Syntactic functions} are expressed by words or groups of words.
		
		\begin{itemize}
			\item E.g., syntactic functions: \expword{subject, object, etc.}
		\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		\item Context-Free grammar (CFG): A widely used formalism to model constituency structure.
		\item Parsing:
		\begin{itemize}
			\item \optword{Bottom-up (ascending)}: Starting from the words, we combine them into phrases using grammar rules until reaching the start symbol \keyword{S}.
			\item \optword{Top-down (descending)}: Starting from \keyword{S}, we apply grammar rules to generate the sentence, guided by the input words.
		\end{itemize}
	\end{itemize}
	...
	\begin{center}
		...
		\hline
		Bottom-up & \optword{CKY}, LR & \optword{Probabilistic CKY}\\
		\hline
		Top-down & Earley, LL, Recursive descent & \\
		\hline
	\end{tabular}
	\end{center}
...
\begin{frame}
	...
	\begin{itemize}
		\item \keyword{Cocke–Kasami–Younger} (CKY) algorithm
	...
	\begin{minipage}{.56\textwidth}
		...
		\tiny
		\begin{algorithm}[H]
			\KwData{A proper grammar $G = (\Sigma, N, \mathcal{P}, S)$}
			\KwResult{CNF grammar $G' = (\Sigma, N', \mathcal{P}', S)$}
			
			Replace all terminals $\sigma$ in each rule's right-hand side by a variable $v$\;
			Add the rule $v \rightarrow\ \sigma$ to $\mathcal{P}'$\;
...
\begin{frame}
	...
	\begin{block}{CKY: Sentence Recognition}
		...
		\begin{algorithm}[H]
			\KwData{A grammar $G = (\Sigma, N, \mathcal{P}, S)$ in CNF; a sentence $w = w_1 \ldots w_n$}
			\KwResult{$T[n, n]$}
			
			\lFor(\tcp*[f]{Initialize the diagonal}){$ i = 1 \ldots n$}{
				$T[i, i] \leftarrow \{ (A, 0, 0, 0) / (A \rightarrow w_i) \in \mathcal{P} \} $ 
			}
			
			\For(\tcp*[f]{Browse rows from bottom to top (no diagonal cells)}){$ i = (n-1) \ldots 2$}{
				\For(\tcp*[f]{Browse columns from diagonal+1 to end}){$ j = (i+1) \ldots n $}{
					\For(\tcp*[f]{Combine left and right spans via rules A → B C}){$ k = i \ldots (j-1) $}{
						\ForEach{$A$ such as $(A \rightarrow B C) \in \mathcal{P} $, $B \in T[i, k]$ and $C \in T[k+1, j]$}{
...
\begin{frame}
	...
	\begin{block}{CKY: Tree Building}
		...
		\begin{algorithm}[H]
			...
			\KwData{Parse table: $T[n, n]$}
			\KwResult{Root of the parse tree:: $r \leftarrow\ \varnothing$}
			...
			\Fn{\FConst{$i, j, pos$}}{
				...
				node.value $\leftarrow\  A$ \;
				\If(\tcp*[f]{Non-terminal: has children}){$k>0$}{
					node.left $\leftarrow$ \FConst{$i, k, iB$}\;
					node.right $\leftarrow$ \FConst{$k+1, j, iC$}\;
				}
				\Return node\;
			}		
	%		\vspace{-3pt}
...
\begin{frame}
	...
	\begin{itemize}
		\item The resulting parse tree is binary due to the use of CNF. 
		However, in practice, the tree should reflect the original grammar.
		\begin{itemize}
			\item Add a post-processing step to recover the original tree structure.
			\item Alternatively, extend the CKY algorithm to handle unit productions.
		\end{itemize}
		\item Syntactic ambiguity
		\begin{itemize}
			\item E.g. \expword{I eat rice with a fork.} vs. \expword{I eat rice with meat.}
	%		\item 
		\end{itemize}
	\end{itemize}
	\end{minipage}
...
\begin{frame}
	...
	\begin{itemize}
		\item Given a probabilistic context-free grammar (PCFG) $G = (\Sigma, N, \mathcal{P}, S)$,
		\item the new rules created by CNF have a probability equal to $1$.
		\item Given a sentence to be parsed $w = w_1 \ldots w_n$,
		\item the probability of a parse tree $T$ given a sentence $w$:
		\[P(T \mid\ w) = \prod\limits_{(A_i \rightarrow \beta_i) \in T} P(A_i \rightarrow \beta_i)\]
		\item Given the different parse trees $\mathcal{T}(w)$ of $w$,
		\item the parse tree is the most probable one:
		\[\hat{T} = \arg\max\limits_{T \in \mathcal{T}(w)} P(T \mid\ w)\]
	\end{itemize}
...
\begin{frame}
	...
	\begin{block}{Probabilistic CKY: Sentence Recognition}
		...
		\begin{algorithm}[H]
			...
			\lFor{$ i = 1 \ldots n$}{
				$T[i, i, A] \leftarrow \{ (P(A \rightarrow w_i), 0, A, A) / (A \rightarrow w_i) \in \mathcal{P} \} $
			}
			
			\For{$ i = (n-1) \ldots 1$ }{
				\For{$ j = (i+1) \ldots n $}{
					\For{$ k = i \ldots (j-1) $}{
						\ForEach{$A$ such as $(A \rightarrow B C) \in \mathcal{P} $, $T[i, k, B] > 0$ and $T[k+1, j, C] > 0$}{
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \textbf{Configuration}:  $C = (\sigma, \beta, A)$ where $\sigma$ is a stack, $\beta$ is an input buffer and $A$ is the set of dependency arcs.
			\item $C_{\text{initial}} = ([\text{ROOT}], w_1 \ldots w_n, \emptyset)$
			\item $C_{\text{final}} = ([\text{ROOT}], \varnothing, A)$
		\end{itemize}
	\end{itemize}
	\end{minipage}
	...
	\begin{minipage}{.6\textwidth}
		\begin{itemize}
			...
			\begin{itemize}
				\item \textbf{Graph}: $G = (V, E)$ where $V$ is a set of nodes (words) and $E$ is the set of possible dependency arcs.
				\item \textbf{Output}: $T = (V, F)$ is a subgraph of $G$ which represents dependency relations.
			\end{itemize}
		\end{itemize}
	\end{minipage}
	\begin{minipage}{.38\textwidth}
...
\begin{frame}
	...
	\begin{minipage}{.6\textwidth}
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item $A$ is the set of dependency arcs
				\item $C_{\text{initial}} = ([\text{ROOT}], w_1 \ldots w_n, \emptyset)$
				\item $C_{\text{final}} = ([\text{ROOT}], \varnothing, A)$
			\end{itemize}
		\end{itemize}
	...
	\begin{itemize}
		\item \textbf{Transition}: a change from one configuration to another using actions on:
		\begin{itemize}
			\item $\sigma$: push or pop a word;
			\item $\beta$: remove the first word or replace it;
			\item $A$: add a dependency arc between two words.
		\end{itemize}
		\item \textbf{Oracle}: a mechanism (rule-based or learned) that decides the next transition.
	\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		\item \texttt{Oracle} model chooses a transition \texttt{t}.
		\item The function \texttt{Apply} executes \texttt{t} on the configuration \texttt{C}'.
	\end{itemize}
	
	\begin{block}{Dependency parsing using transitions}
		\begin{algorithm}[H]
			\KwData{The sentence to be parsed: $w_1 w_2 \ldots w_n$}
			\KwResult{Set of dependency arcs: $A$}
			
			$C \leftarrow\ (\sigma=[ROOT], \beta = w_1 w_2 \ldots w_n, A = \emptyset)$\;
...
\begin{frame}
	...
	\begin{itemize}
		\item \texttt{Oracle} model learns to predict the transition $\hat{t}$ given:
		\begin{itemize}
			\item current configuration: $ C = (\sigma, \beta, A) $;
			\item the set of predicted transitions $T$;
			\item a function $\Psi$ which calculates a score using features extracted from the configuration;
			\item an annotated text (Treebank).
		\end{itemize}
		\[ \hat{t} = \arg\max\limits_{t \in T} \Psi(t, C, w; \theta) \]
	
		\item The annotated text must be transformed to a sequence of transitions.
		\item To train the \texttt{Oracle} model, the parsing algorithm is used to generate gold transition sequences.
		\item The function $\Psi$ can be implemented using MaxEnt (most common), SVM, or neural networks.
	\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item \textbf{Set of dependency arcs $A$}:
		\begin{itemize}
			\item The dependency arcs created so far.
		\end{itemize}
	
		\item \textbf{Sentence-level features}:
		\begin{itemize}
			\item The distance between the top word of the stack and the first word in the buffer (number of words between them in the sentence).
		\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		\item \optword{SHIFT}: Move the first word from the buffer to the stack.
		\[ (\sigma, w_i|\beta, A) \Rightarrow  (\sigma|w_i, \beta, A) \]
		
		\item \optword{ARC-LEFT}: Create an arc from the first word in the buffer to the top of the stack.
		\[ (\sigma|w_i, w_j|\beta, A) \Rightarrow  (\sigma, w_j|\beta, A \cup \{w_j \rightarrow w_i \}) \] 
		
		\item \optword{ARC-RIGHT}: Create an arc from the top of the stack to the first word in the buffer.
...
\begin{frame}
	...
	\begin{exampleblock}{Example of unlabeled derivations of the sentence ``\textit{He writes the solution and its explanation}'' using Arc-standard}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item \optword{ARC-LEFT}: Create an arc from the first word in the buffer to the top of the stack.
		\[ (\sigma|w_i, w_j|\beta, A) \xRightarrow{\nexists w_k : (w_k \rightarrow w_i) \in A}  (\sigma, w_j|\beta, A \cup \{w_j \rightarrow w_i \}) \] 
		
		\item \optword{ARC-RIGHT}: Create an arc from the top of the stack to the first word in the buffer.
		\[ (\sigma|w_i, w_j|\beta, A) \Rightarrow  (\sigma|w_i w_j, \beta, A \cup \{w_i \rightarrow w_j \}) \] 
		
		\item \optword{REDUCE}: Pop the top of the stack if it already has a head.
...
\begin{frame}
	...
	\vskip-6pt
	\begin{exampleblock}{Example of unlabeled derivations of the sentence ``\textit{He writes the solution and its explanation}'' using Arc-eager}
		\centering\bfseries\fontsize{5.5}{10}\selectfont
		\begin{tblr}{
				colspec = {llll},
				row{odd} = {lightblue},
...
\begin{frame}
	...
	\begin{minipage}{.6\textwidth}
		\begin{itemize}
			...
			\begin{itemize}
				\item \textbf{Graph}: $G = (V, E)$ where $V$ is the set of nodes (words) and $E$ is the set of arcs (dependency relations).
				\item \textbf{Output}: $T = (V, F)$ is a subgraph of $G$ which represents dependency relations.
			\end{itemize}
		\end{itemize}
	\end{minipage}
	...
	\begin{itemize}
		\item $\Psi$ is a scoring function, and $ \mathcal{T}(G) $ is the set of all valid dependency trees over $G$.
		\[ \hat{T} = \arg\max\limits_{T \in \mathcal{T}(G)} \Psi(T, w; \theta) \]
		
		\item The global score is computed as the sum of scores of individual arcs:
		\[ \Psi(T, w; \theta) = \sum_{e \in F,\, T = (V, F)} \psi(e, w; \theta) \]
		
		\item where $\psi$ is a local scoring function for individual arcs.
	\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Some features:
		\begin{itemize}
			\item Syntactic head, its POS, its lemma, its prefixes and suffixes;
			\item Arc direction (left or right);
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Arcs are weighted where the weight of an arc $e$ is denoted by $G.w(e)$
		\item A \keyword{maximum spanning tree} is constructed from $G$
		\item \keyword{maximum spanning tree}: A directed spanning tree rooted at ROOT where each node (except ROOT) has exactly one incoming arc, and the sum of arc weights is maximized.
	...
	\begin{block}{Chu-Liu-Edmonds parsing: Maximum spanning tree}
		...
		\begin{algorithm}[H]
			...
			\Fn{\MST{$G, ROOT$}}{
				...
				\ForEach{$v \in V \setminus \{ROOT\}$}{
					$bestInArc \leftarrow \arg\max_{(u,v) \in E} G.w(u,v)$;
					$F \leftarrow F \cup \{bestInArc\}$\;
					\lForEach{$(u,v) \in E$}{
						$G.w(u,v) \leftarrow G.w(u,v) - G.w(bestInArc)$;
					}
				}
				
				\eIf{$F$ contains no cycle}{
					\Return $(V, F)$\;
				}
				{
					$C \leftarrow$ a cycle in $F$;
					$G' \leftarrow Contract(G, C)$\;
					$T' \leftarrow \MST(G', ROOT)$\;
...
\begin{frame}
	\frametitle{\insertshortsubtitle: \insertsection}
	\framesubtitle{\insertsubsection: Chu-Liu/Edmonds parsing (Exercise solution)}
	
	\begin{figure}
		\centering
		\hgraphpage[.7\textwidth]{exp-graph-parsing.pdf}
		\caption{Chu-Liu/Edmonds parsing of the sentence ``Read the course'' }
	\end{figure}
